...
13. [Chat Context Dependencies](#13-chat-context-dependencies)
14. [Key Patterns & Gotchas](#14-key-patterns--gotchas)
...
## 13. Chat Context Dependencies

Chat quality depends on having the right caches populated. The app builds a context object from cached data and sends it to the edge function (`quick-processor`) alongside the user's messages.

### Book Chat (`BookChatContext`)

Built in `ChatPage.tsx` when opening chat for a book. The edge function uses this context to ground the AI's responses.

| Context Field | Source Cache Table | Grok Call? | Priority |
|---|---|---|---|
| `summary` | `book_summary_cache` | Yes | **High** — core book understanding |
| `insights.authorFacts` | `author_facts_cache` | Yes | Medium |
| `insights.influences` | `book_influences_cache` | Yes | Medium |
| `insights.domain` | `book_domain_cache` | Yes | Medium |
| `insights.context` | `book_context_cache` | Yes | Medium |
| `insights.didYouKnow` | `did_you_know_cache` | Yes | Medium |
| `podcasts` | `podcast_episodes_cache` | Yes (curated) | Low — referenced in responses |
| `videos` | `youtube_videos` | No (YouTube API) | Low |
| `articles` | `google_scholar_articles` | No (Google Scholar) | Low |
| `relatedBooks` | `related_books` | Yes | Low |
| `relatedWorks` | `related_movies` | No (TMDB + Odesli) | Low |
| `discussionQuestions` | `discussion_questions_cache` | Yes | Medium — drives conversation |

Non-cache fields (from `books` table directly): `title`, `author`, `genre`, `publishYear`, `readingStatus`, `userNotes`, `userRatings`.

**Minimum for useful chat:** `book_summary_cache` + at least one insight type. Without any caches, chat falls back to just title/author/genre which produces generic responses.

### Character Chat (`CharacterChatContext`)

Built when opening chat with a specific character (e.g., Jay Gatsby).

| Context Field | Source | Grok Call? |
|---|---|---|
| `characterName` | `character_avatars_cache.avatars[].name` | Yes (character identification) |
| `context` | `character_avatars_cache.contexts[characterName]` | Yes (structured character JSON) |
| `avatarUrl` | `character_avatars_cache.avatars[].image_url` | Yes (Replicate image gen) |
| `bookTitle`, `bookAuthor` | `books` table | No |

**The `character_avatars_cache` is the single dependency.** The `contexts` field contains a structured JSON per character with role, personality, relationships, speech patterns, etc. — all generated by a single Grok call. Without it, character chat cannot function.

### Admin Tool Implications

When bulk-populating caches for chat readiness, populate in this order:
1. `book_summary_cache` (required for book chat)
2. `character_avatars_cache` (required for character chat)
3. `discussion_questions_cache` (drives conversation topics)
4. Insight caches: `did_you_know`, `book_context`, `author_facts`, `book_influences`, `book_domain`
5. Media caches: `podcast_episodes_cache`, `youtube_videos`, `google_scholar_articles`, `related_books`, `related_movies`
