...
const NetflixIcon = () => (
  <svg viewBox="0 0 24 24" width="20" height="20" fill="#E50914">
...
export default function WatchModal({ watchLinks, title, year, onClose }: WatchModalProps) {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[9999] bg-black/40 backdrop-blur-sm flex items-end justify-center px-4"
        onClick={onClose}
      >
        <motion.div
          initial={{ y: '100%' }}
          ...
          className="w-full max-w-md rounded-t-3xl shadow-2xl relative"
          style={{ background: 'rgba(15, 23, 42, 0.8)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(255, 255, 255, 0.1)', borderBottom: 'none' }}
        ...
        >
          ...
          <div className="px-5 pb-4">
            ...
            <div className="text-center mb-4">
              <p className="text-sm font-bold text-white truncate">{title || 'Watch on...'}</p>
              ...
              {available.map((platform, i) => {
                ...
                return (
                  <motion.button
                    ...
                    className="flex items-center gap-3 w-full px-4 py-3 rounded-xl active:scale-[0.98] transition-all"
            ...
            {watchLinks.tmdb_url && (
              <button
                onClick={() => { openLink(watchLinks.tmdb_url!); onClose(); }}
                className="w-full text-center mt-3 text-[11px] text-slate-500 active:text-slate-300"
              >
          ...
          <div style={{ paddingBottom: 'calc(16px + var(--safe-area-bottom, 0px))' }} />
        </motion.div>
      </motion.div>
    </AnimatePresence>
  );
