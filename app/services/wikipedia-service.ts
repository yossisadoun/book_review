...
import { fetchWithRetry, first4DigitYear } from './api-utils';
import { isHebrew } from '../components/utils';

export async function getWikidataItemForTitle(pageTitle: string, lang = 'en'): Promise<string | null> {
...
export async function getAuthorAndYearFromWikidata(qid: string, lang = 'en'): Promise<{ author: string; publishYear?: number; genre?: string; isbn?: string }> {
...
export async function lookupBooksOnWikipedia(query: string): Promise<Omit<Book, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'rating_writing' | 'rating_insights' | 'rating_flow' | 'rating_world' | 'rating_characters'>[]> {
...
export async function lookupBookOnWikipedia(query: string): Promise<Omit<Book, 'id' | 'user_id' | 'created_at' | 'updated_at' | 'rating_writing' | 'rating_insights' | 'rating_flow' | 'rating_world' | 'rating_characters'> | null> {
  const lang = isHebrew(query) ? 'he' : 'en';
