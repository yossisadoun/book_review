...
exports.generateFeedItemsForBook = generateFeedItemsForBook;
exports.getPersonalizedFeed = getPersonalizedFeed;
exports.markFeedItemsAsShown = markFeedItemsAsShown;
exports.markFeedItemAsShown = markFeedItemAsShown;
exports.createFriendBookItem = createFriendBookItem;
exports.regenerateFeedForUser = regenerateFeedForUser;
// Generate a hash for content deduplication (browser-compatible)
async function generateContentHash(type, content) {
...
function generateContentHashSync(type, content) {
    const str = JSON.stringify({ type, content });
    let hash = 5381;
 ...
 */
async function generateFeedItemsForBook({ client, userId, book, }) {
    const feedItems = [];
    const errors = [];
    ...
    const [authorFacts, bookContext, bookDomain, bookInfluences, podcasts, articles, relatedBooks, youtubeVideos,] = await Promise.all([
    ...
    if (bookDomain?.domain_insights && Array.isArray(bookDomain.domain_insights)) {
        ...
        for (const insight of bookDomain.domain_insights) {
            feedItems.push(createFeedItem(userId, book, 'drilldown', {
                insight,
...
function createFeedItem(userId, book, type, content) {
    return {
        user_id: userId,
...
async function fetchCacheData(client, table, title, author) {
    try {
 ...
 */
async function getPersonalizedFeed({ client, userId, limit = 20, includeFriendBooks = true, }) {
 ...
 */
function calculateFeedScore(item, recentTypes, recentBooks) {
...
async function fetchFeedItemsByType(client, userId, type, limit) {
    const { data, error } = await client
        .from('feed_items')
        .select('*')
        .eq('user_id', userId)
...
async function fetchFriendBookItems(client, userId, limit) {
 ...
 */
async function markFeedItemsAsShown(client, itemIds) {
    if (itemIds.length === 0)
        return;
 ...
 */
async function markFeedItemAsShown(client, itemId) {
 ...
 */
async function createFriendBookItem(client, userId, book) {
    const content = {
        action: 'added',
 ...
 */
async function regenerateFeedForUser(client, userId) {
