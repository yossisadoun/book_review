"use strict";
// Prompt loading utility (works for both web and mobile)
Object.defineProperty(exports, "__esModule", { value: true });
exports.loadPrompts = loadPrompts;
exports.formatPrompt = formatPrompt;
// Default prompts (fallback if file can't be loaded)
...
- If the input is Hebrew, search for Hebrew books and return Hebrew titles/authors`,
    ...
    author_facts: {
        prompt: `You are a literary expert. Generate exactly 20 interesting, fun facts about the author "{author}" specifically in the context of their book "{bookTitle}" or about the book itself.

Requirements:
- Return exactly 20 facts
...
- add a url for the reference where you got the fact when relevant.
- Also provide the original publication year (earliest known) for this book in its earliest version. This should be the year the book was first published in any language or format.
- Return ONLY valid JSON in this format: { "facts": ["Fact 1", "Fact 2", ..., "Fact 20"], "first_issue_year": 1925 }`,
    },
    book_influences: {
...
2. The Book Review by The New York Times (https://podcasts.apple.com/us/podcast/the-book-review/id120315179)
...
async function loadPrompts(forceReload = false) {
    // For mobile/web compatibility, use defaults
    // In production, you could load from a CDN or bundled file
    if (!forceReload && promptsCache) {
        return promptsCache;
    }
    // For now, always return defaults
    // TODO: Add YAML parsing if loading from URL/file is needed
