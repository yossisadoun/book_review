...
import { fetchWithRetry, first4DigitYear } from '../utils/fetch-retry';
import { isHebrew } from '../utils/hebrew-detector';
...
export async function getAuthorAndYearFromWikidata(
  qid: string,
  lang = 'en'
): Promise<{ author: string; publishYear?: number; genre?: string; isbn?: string }> {
...
export async function lookupBooksOnWikipedia(
  query: string
...
export async function lookupBookOnWikipedia(
  query: string
